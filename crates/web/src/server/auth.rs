...
{
    let r = sjf_auth::endpoints::terminal_challenge(def::get_tower_session().await?, username).await;
    to_err(r)

}

#[server(endpoint="passkey/registration/start",input=dioxus::prelude::server_fn::codec::PostUrl)]
pub async fn start_registration(username: String, terminal_challange: String) -> Result<webauthn_rs_proto::CreationChallengeResponse ,ServerFnError<AuthApiError>> 
{
    let r = sjf_auth::endpoints::start_register(def::get_state().await?, def::get_tower_session().await?, username, terminal_challange).await;
    to_err(r)

}
#[server(endpoint="passkey/registration/finish",input=dioxus::prelude::server_fn::codec::PostUrl)]
pub async fn finish_registration(reg: webauthn_rs_proto::RegisterPublicKeyCredential) -> Result<() ,ServerFnError<AuthApiError>> 
{
    let r = sjf_auth::endpoints::finish_register(def::get_state().await?, def::get_tower_session().await?, reg).await;
    to_err(r)

}
#[server(endpoint="passkey/authentication/start",input=dioxus::prelude::server_fn::codec::PostUrl)]
pub async fn start_authentication(username: String) -> Result<webauthn_rs_proto::RequestChallengeResponse ,ServerFnError<AuthApiError>> 
{
    let r = sjf_auth::endpoints::start_authentication(def::get_state().await?, def::get_tower_session().await?, username).await;
    to_err(r)
}
#[server(endpoint="passkey/authentication/finish",input=dioxus::prelude::server_fn::codec::PostUrl)]
pub async fn finish_authentication(key: webauthn_rs_proto::PublicKeyCredential) -> Result<() ,ServerFnError<AuthApiError>> 
{
    let uuid = to_err(sjf_auth::endpoints::finish_authentication(def::get_state().await?, def::get_tower_session().await?, key).await)?;
    let mut session = def::get_session().await?;
    let user = match to_err(session.authenticate( sjf_auth::Credentials { uuid} ).await)? {
        Some(user) => user,
        None => {
            return Err(ServerFnError::ServerError("invalid".into()))
        }
    };
