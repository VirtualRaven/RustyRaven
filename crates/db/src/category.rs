...
use sqlx::{Executor, Postgres, query, query_as, query_file};
use tracing::error;
...
pub async fn create(req: CreateReq) -> Result<CreateRsp, sqlx::Error> {
    ...
    query!(
        "INSERT INTO product_categories_hierarchy (ancestor,descendant,depth) VALUES($1,$1,$2)",
        res.id,
        depth
    )
    .execute(&mut *tx)
...
pub async fn delete(req: DeleteReq) -> Result<(), sqlx::Error> {
    ...
    query!(
        "DELETE FROM product_categories_hierarchy where descendant=$1",
        req.id as i32
    )
    .execute(&mut *tx)
    .await?;
...
pub async fn get_child_categories<'c, E>(
    root: Option<u32>,
    recursive: bool,
    e: E,
...
pub async fn update_name(id: u32, name: String) -> Result<(), sqlx::Error> {
    query!(
        "UPDATE product_categories SET name=$1 where id=$2 RETURNING ID",
        name,
        id as i32
    )
...
{
    if create {
        query_file!("sql/materialized_product_paths.sql")
            .execute(e)
            .await?;
        query!("CREATE UNIQUE INDEX IF NOT EXISTS product_paths_index  on product_paths (id)")
            .execute(e)
            .await?;
    }

    query!("REFRESH MATERIALIZED VIEW CONCURRENTLY product_paths")
        .execute(e)
        .await?;
    Ok(())
}
...
pub async fn get_paths() -> Result<HashMap<String, u32>, sqlx::Error> {
    ...
    Ok(res
        .into_iter()
        .map(|r| (r.path.unwrap(), r.id.unwrap() as u32))
        .collect())
}
